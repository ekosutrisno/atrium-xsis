<template>
  <div class="flex relative w-full">
   <div class="flex-1">
      <header class="p-4 relative pt-[18px] max-w-2xl flex justify-between bg-white dark:bg-color-dark-gray-darker shadow rounded-lg">
         <div class="text-color-dark-black-default dark:text-color-gray-light">
            ...
            <div class="text-color-gray-darkest dark:text-color-gray-default flex flex-col mt-2.5 text-sm">
              ...
              <span class="font-semibold text-indigo-600 dark:text-indigo-300 "> {{ currentUser.clients[0].clientName }} </span>
            </div>
         </div>
      ...
      <div class="md:grid-cols-3 gap-5 grid w-full">
        <header class="p-4 sm:sticky top-6 max-h-44 md:col-span-1 pt-[18px] flex justify-between bg-white dark:bg-color-dark-gray-darker shadow rounded-lg">
          <div class="text-color-dark-black-default dark:text-color-gray-light">
                  ...
                  <span>Enable dark mode</span>
                    <div class="inline-flex items-center space-x-4 mt-3">
                      <button type="button" v-if="theme == 'dark'" @click="togleDarkLightMode('light')"
                        class="text-[#9a6fc3] py-2 px-4 text-xs rounded border-[#9a6fc3]  dark:hover:bg-color-dark-gray-darkest border"
                      >
                      ...
                      <button v-else type="button" @click="togleDarkLightMode('dark')"
                        class="text-[#9a6fc3] py-2 px-4 text-xs rounded border-[#9a6fc3] hover:bg-[#f0e7f8] border"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <p class="text-color-dark-gray-darker" v-if="theme != 'dark'">Light theme enabled.</p>
                      <p class="text-color-gray-light" v-else >Dark theme enabled.</p>
                  </div>
              </div>
          </div>
        </header>
